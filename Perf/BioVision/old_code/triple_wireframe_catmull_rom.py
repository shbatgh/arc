#Triple wireframe, except uses catmull_rom interpolation to create the wireframes.
wf_height = 3/0.198
...
def new_sorted_outlines(outline_list):
    sorted_outlines = outline_list.copy()
...
def create_wf_list(sorted_outlines, plane_val, z_start):        #Going up then down
    ...
    #---Combining, only take points from up_dict that are also in down_dict
    ...
    wf_list = up_list + down_list[::-1]  #Going down, so reverse the list


    #Maybe put something in between here to make the meshes more complex

    #MUST BE SPLINED LATER
    
    return(wf_list)


def triple_wireframe_creation(outline_list, x_or_y, starting_slice, wf_dist_arg, wf_offset_arg, points_per_segment):
    ...
    #print(plane_vals, "\n")
    sorted_outlines = new_sorted_outlines(outline_list)
    ...
    for val in plane_vals:
        ...
        if len(wf_list)<3:  #Making sure wires are at least 3 points
            print("Not enough points for splining at plane value: ", val, "    axis: ", comp)
