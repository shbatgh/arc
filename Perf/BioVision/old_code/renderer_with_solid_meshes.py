...
def add_step_box(x,y,z, collection, color = None):
    #CODE TO ADD BOXES AT MESH INTERSECTIONS
...
def add_curve(frames_dict, color_dict, coords, name, slice, frame, color):
    ...
    curveData.bevel_depth      = depth   # Bevel Depth
    ...
    for i, coord in enumerate(coords):
        ...
        if len(coord) >2:
            ...
            #add_step_box(x,y,z, cur_frame_col)
            
        else:
            z = frames_dict[frame].index(slice)*(3/0.198)
            

        polyline.points[i].co = (x/zoom_in_factor, y/zoom_in_factor, z/zoom_in_factor, 1)
    
    if mesh_marker:
        curveData.bevel_depth = depth
        curveData.resolution_u = 1      #DOESNT CHANGE NOW
...
def add_solid_mesh(vertices, faces, name, collection):
...
def add_tracer(coords, name, collection):
    curveData = bpy.data.curves.new('my_curve', type='CURVE')
    curveData.dimensions = '3D'
    curveData.resolution_u = 3     # Preview U
    curveData.fill_mode = 'FULL' # Fill Mode ==> Full
    curveData.bevel_depth      = depth   # Bevel Depth
    curveData.bevel_resolution = 1      # Bevel Resolution
    ...
    curveOB = bpy.data.objects.new(name, curveData)         # create Object
    collection.objects.link(curveOB)                     #add to collection
...
def get_data(file_path):
    ...
    data_str = "".join(lines[1:] if (header == "MESH" or header == "TRACER") else lines)
    
    parsed_data = eval(data_str, {"__builtins__": None}, {})
...
def render_curves(frames_dict, cur_render_col, cur_rendering_group):
    global color_dict
    global cur_frame_col
...
def render_meshes(mesh_frames_dict, color_dict, parent_collection, rendering_group):
    ...
    for frame_num, mesh_objs in mesh_frames_dict.items():
        ...
        for obj in mesh_objs:
            ...
            mesh.data.materials.append(color_dict[color])
            print("Mesh Created")



def render_tracers(tracer_data, color_dict, parent_collection):
    t_num = -1
    for color in tracer_data.keys():
        ...
        for cur_tracer in tracer_data[color]:
            ...
            tracer_obj = add_tracer(
                coords=cur_tracer,
                name='T' + str(t_num) + ' ' + str(color),
                collection=parent_collection
                )
            tracer_obj.data.materials.append(color_dict[color])
...
def render():
    ...
    header, parsed_data = get_data(data_file_path)
    header_name = header
    if header not in ["MESH", "TRACER"]:
        header_name = "WIREFRAME"

    print("Got Data" + header)
    cur_render_col = bpy.data.collections.new(header_name + ' Rendering '+str(cur_render_number))                 #Collection for each time a file is rendered. All tps are added to it
    ...
    if header == "MESH":
        render_meshes(parsed_data, color_dict, cur_render_col, cur_rendering_group)
    elif header == "TRACER":
        render_tracers(parsed_data, color_dict, cur_render_col)
    else:
        render_curves(parsed_data, cur_render_col, cur_rendering_group)
...
last_tp_index = -1  # Global state
#Change later to by name
def my_handler(scene):
    global last_tp_index
    frame = scene.frame_current
    tp_index = frame // frames_per_time_point

    if (tp_index == last_tp_index) or (tp_index not in tp_frames):
        return  # Same timepoint, no need to update

    last_tp_index = tp_index

    # Hide all
    for frame_collections in tp_frames.values():
        for col in frame_collections:
            col.hide_viewport = True
    for col in tp_frames[tp_index]:
...
classes = [VIEW3D_PT_ColorPanel, MESH_OT_a_hide_color, MESH_OT_a_show_color, MESH_OT_a_show_all, MESH_OT_a_show_only, VIEW3D_PT_RenderPanel, MESH_OT_a_render, MESH_OT_a_choose_path]
def register():
